...
// 1. Đảm bảo đã import supabase client
...
export default function GrammarDetail() {
  ...
  // 2. Cố định lesson_no là 2 cho bài học này
  const lessonNo = 2; 
  ...
  useEffect(() => {
    async function fetchGrammarData() {
      try {
        ...
        // 3. Truy vấn Supabase lấy các ví dụ của bài 2
        const { data, error } = await supabase
          .from('grammars')
          .select('*')
          .eq('lesson_no', lessonNo)
          .order('id', { ascending: true }); // Sắp xếp theo ID cho thứ tự ổn định
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 md:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        {/* --- NỘI DUNG NGỮ PHÁP (GIỮ NGUYÊN STYLE BẠN ĐÃ VIẾT) --- */}
            ...
            <div className="space-y-6">
              ...
              {examples.length === 0 && (
                <div className="text-center py-20 bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
                  ...
                  <p className="text-gray-300">Chưa có ví dụ nào cho bài 2.</p>
                </div>
              )}
            </div>
          )}
